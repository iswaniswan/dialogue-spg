...
class Perusahaan extends CI_Controller
{
	...
	public function save()
	{
		...
		$this->form_validation->set_rules('ecompany', 'ecompany', 'trim|required|min_length[0]');
		if ($this->form_validation->run() == false) {
			...
			$cek = $this->mymodel->cek($this->input->post('ecompany', TRUE));
			...
			if ($cek->num_rows() > 0) {
				...
				$this->db->trans_begin();
				$ecompany = $this->input->post('ecompany', TRUE);
				$this->mymodel->save($ecompany);
				if ($this->db->trans_status() === FALSE) {
					...
					$this->logger->write('Simpan Data '.$this->title.' : '.$ecompany);
					$data = array(
						'sukses' => true,
						'ada'	 => false,
					);
	...
	public function update()
	{
		...
		$this->form_validation->set_rules('ecompanyold', 'ecompanyold', 'trim|required|min_length[0]');
		$this->form_validation->set_rules('ecompany', 'ecompany', 'trim|required|min_length[0]');
		$icompany 		= $this->input->post('icompany', TRUE);
		$ecompanyold 	= $this->input->post('ecompanyold', TRUE);
		$ecompany 		= $this->input->post('ecompany', TRUE);
		if ($this->form_validation->run() == false) {
			$data = array(
				'sukses' => false,
			...
			/** Cek Jika Nama Sudah Ada */
			$cek = $this->mymodel->cek_edit($ecompany,$ecompanyold);
			/** Jika Sudah Ada Jangan Disimpan */
			if ($cek->num_rows() > 0) {
				$data = array(
					'sukses' => false,
				...
				$this->db->trans_begin();				
				$this->mymodel->update($icompany,$ecompany);
				if ($this->db->trans_status() === FALSE) {
					$this->db->trans_rollback();
					$data = array(
						'sukses' => false,
					...
					$this->db->trans_commit();
					$this->logger->write('Update Data '.$this->title.' : '.$ecompany);
					$data = array(
						'sukses' => true,
						'ada'	 => false,
					);
