...
class Power extends CI_Controller
{
	...
	public function save()
	{
		...
		$this->form_validation->set_rules('epower', 'epower', 'trim|required|min_length[0]');
		if ($this->form_validation->run() == false) {
			...
			$cek = $this->mymodel->cek($this->input->post('epower', TRUE));
			...
			if ($cek->num_rows() > 0) {
				...
				$this->db->trans_begin();
				$epower  = ucwords($this->input->post('epower', TRUE));
				$this->mymodel->save($epower);
				if ($this->db->trans_status() === FALSE) {
					...
					$this->logger->write('Simpan Data '.$this->title.' : '.$epower);
					$data = array(
						'sukses' => true,
						'ada'	 => false,
					);
	...
	public function update()
	{
		...
		$this->form_validation->set_rules('ipower', 'ipower', 'trim|required|min_length[0]');
		$this->form_validation->set_rules('epowerold', 'epowerold', 'trim|required|min_length[0]');
		$this->form_validation->set_rules('epower', 'epower', 'trim|required|min_length[0]');
		$ipower 	= $this->input->post('ipower', TRUE);
		$epowerold 	= $this->input->post('epowerold', TRUE);
		$epower 	= ucwords($this->input->post('epower', TRUE));
		if ($this->form_validation->run() == false) {
			$data = array(
				'sukses' => false,
			...
			/** Cek Jika Nama Sudah Ada */
			$cek = $this->mymodel->cek_edit($epower,$epowerold);
			/** Jika Sudah Ada Jangan Disimpan */
			if ($cek->num_rows() > 0) {
				$data = array(
					'sukses' => false,
				...
				$this->db->trans_begin();				
				$this->mymodel->update($ipower,$epower);
				if ($this->db->trans_status() === FALSE) {
					$this->db->trans_rollback();
					$data = array(
						'sukses' => false,
					...
					$this->db->trans_commit();
					$this->logger->write('Update Data '.$this->title.' : '.$epower);
					$data = array(
						'sukses' => true,
						'ada'	 => false,
					);
