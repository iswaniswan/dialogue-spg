...
class Level extends CI_Controller
{
	...
	public function save()
	{
		...
		$this->form_validation->set_rules('elevel', 'elevel', 'trim|required|min_length[0]');
		if ($this->form_validation->run() == false) {
			...
			$cek = $this->mymodel->cek($this->input->post('elevel', TRUE));
			...
			if ($cek->num_rows() > 0) {
				...
				$this->db->trans_begin();
				$elevel  = ucwords($this->input->post('elevel', TRUE));
				$deskripsi = $this->input->post('deskripsi', TRUE);
				$this->mymodel->save($elevel,$deskripsi);
				if ($this->db->trans_status() === FALSE) {
					...
					$this->logger->write('Simpan Data '.$this->title.' : '.$elevel);
					$data = array(
						'sukses' => true,
						'ada'	 => false,
					);
	...
	public function update()
	{
		...
		$this->form_validation->set_rules('elevelold', 'elevelold', 'trim|required|min_length[0]');
		$this->form_validation->set_rules('elevel', 'elevel', 'trim|required|min_length[0]');
		$ilevel 	= $this->input->post('ilevel', TRUE);
		$elevelold 	= $this->input->post('elevelold', TRUE);
		$elevel 	= ucwords($this->input->post('elevel', TRUE));
		$deskripsi 	= $this->input->post('deskripsi', TRUE);
		if ($this->form_validation->run() == false) {
			$data = array(
			...
			/** Cek Jika Nama Sudah Ada */
			$cek = $this->mymodel->cek_edit($elevel,$elevelold);
			/** Jika Sudah Ada Jangan Disimpan */
			if ($cek->num_rows() > 0) {
				$data = array(
					'sukses' => false,
				...
				$this->db->trans_begin();				
				$this->mymodel->update($ilevel,$elevel,$deskripsi);
				if ($this->db->trans_status() === FALSE) {
					$this->db->trans_rollback();
					$data = array(
						'sukses' => false,
					...
					$this->db->trans_commit();
					$this->logger->write('Update Data '.$this->title.' : '.$elevel);
					$data = array(
						'sukses' => true,
						'ada'	 => false,
					);
