...
class Alasan extends CI_Controller
{
	...
	public function save()
	{
		...
		$this->form_validation->set_rules('ealasan', 'ealasan', 'trim|required|min_length[0]');
		if ($this->form_validation->run() == false) {
			...
			$cek = $this->mymodel->cek($this->input->post('ealasan', TRUE));
			...
			if ($cek->num_rows() > 0) {
				...
				$this->db->trans_begin();
				$ealasan = ucwords($this->input->post('ealasan', TRUE));
				$this->mymodel->save($ealasan);
				if ($this->db->trans_status() === FALSE) {
					...
					$this->logger->write('Simpan Data '.$this->title.' : '.$ealasan);
					$data = array(
						'sukses' => true,
						'ada'	 => false,
					);
	...
	public function update()
	{
		...
		$this->form_validation->set_rules('ialasan', 'ialasan', 'trim|required|min_length[0]');
		$this->form_validation->set_rules('ealasanold', 'ealasanold', 'trim|required|min_length[0]');
		$this->form_validation->set_rules('ealasan', 'ealasan', 'trim|required|min_length[0]');
		$ialasan 		= $this->input->post('ialasan', TRUE);
		$ealasanold 	= $this->input->post('ealasanold', TRUE);
		$ealasan 		= ucwords($this->input->post('ealasan', TRUE));
		if ($this->form_validation->run() == false) {
			$data = array(
				'sukses' => false,
			...
			/** Cek Jika Nama Sudah Ada */
			$cek = $this->mymodel->cek_edit($ealasan,$ealasanold);
			/** Jika Sudah Ada Jangan Disimpan */
			if ($cek->num_rows() > 0) {
				$data = array(
					'sukses' => false,
				...
				$this->db->trans_begin();				
				$this->mymodel->update($ialasan,$ealasan);
				if ($this->db->trans_status() === FALSE) {
					$this->db->trans_rollback();
					$data = array(
						'sukses' => false,
					...
					$this->db->trans_commit();
					$this->logger->write('Update Data '.$this->title.' : '.$ealasan);
					$data = array(
						'sukses' => true,
						'ada'	 => false,
					);
